...
#pragma mark initialize side bar
  ...
  self.bmkMapView = [[BMKMapView alloc] initWithFrame:self.mapView.frame];
...
- (void)didUpdateUserLocation:(BMKUserLocation *)userLocation {
  ...
  // [self.bmkMapView setCenterCoordinate:userLocation.location.coordinate
  // animated:YES];

  [self.bmkMapView setRegion:BMKCoordinateRegionMakeWithDistance(
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  self.bmkMapView.delegate =
      self; // 此处记得不用的时候需要置nil，否则影响内存的释放
  ...
  if (!city && !self.selectedCity) {
    ...
    UIViewController *myView =
        [story instantiateViewControllerWithIdentifier:@"cityNavigator"];
    ...
    if (!self.selectedCity) {
      ...
      [[NSUserDefaults standardUserDefaults] setValue:self.selectedCity.cityName
                                               forKey:@"city"];
    }
  }
}
...
- (void)viewDidAppear:(BOOL)animated {
  if (self.selectedCity) {
    NSString *title = [[NSString alloc]
        initWithFormat:@"%@%@", self.selectedCity.cityName, @"地铁线路"];
...
- (void)initialData {
  ...
  NSString *database_path =
      [[NSBundle mainBundle] pathForResource:@"metroinfo" ofType:@"sqlite"];
  NSLog(database_path);
  ...
  NSString *sqlQuery =
      [[NSString alloc] initWithFormat:@"select * from %@", TABLENAME];
  sqlite3_stmt *statement;

  if (sqlite3_prepare_v2(db, [sqlQuery UTF8String], -1, &statement, nil) !=
      SQLITE_OK) {
    sqlite3_close(db);
    NSLog(@"database open error");
    [self Alert:@"数据库读取异常"];
    ...
    while (sqlite3_step(statement) == SQLITE_ROW) {
      ...
      NSString *openDate1 =
          [[NSString alloc] initWithUTF8String:(const char *)open1];
      const unsigned char *open2 = sqlite3_column_text(statement, 4);
      NSString *openDate2 =
          [[NSString alloc] initWithUTF8String:(const char *)open2];
      const unsigned char *close1 = sqlite3_column_text(statement, 5);
      NSString *closeDate1 =
          [[NSString alloc] initWithUTF8String:(const char *)close1];
      const unsigned char *close2 = sqlite3_column_text(statement, 6);
      NSString *closeDate2 =
          [[NSString alloc] initWithUTF8String:(const char *)close2];
      int lineId = sqlite3_column_int(statement, 7);
      MetroStation *stationItem =
          [[MetroStation alloc] initWithName:stationName
                                   openDate1:openDate1
      ...
      NSUInteger indexOfLine = [self.metroArray
      ...
      if (indexOfLine == NSNotFound) {
        MetroLineItem *line = [[MetroLineItem alloc] initWithName:lineName
                                                           lineId:lineId
                                                         Stations:nil];
...
- (NSInteger)tableView:(UITableView *)tableView
  ...
  if ([tableView isEqual:self.metroSearchController
                             .searchResultsTableView]) { // searched result
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  if ([tableView isEqual:self.metroSearchController.searchResultsTableView]) {
    ...
    cell = [tableView dequeueReusableCellWithIdentifier:@"metroLineTableCell"
                                           forIndexPath:indexPath];
    if (self.isStation) {
      cell.textLabel.text = [self.showArray objectAtIndex:indexPath.row];
      NSString *imageString =
          [NSString stringWithFormat:@"icon_metro_line%d", self.iSelectedLine];
      ...
      NSString *imageString =
          [NSString stringWithFormat:@"icon_metro_line%d", (int)item.lineId];
      cell.imageView.image = [UIImage imageNamed:imageString];
    }
  }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
  if ([sender isKindOfClass:UIBarButtonItem.class]) {
    ...
    MetroInfoViewController *vcMetroInfoController =
    ...
    MetroStation *station =
        [item.stationArray objectAtIndex:self.iSelectedStation];
...
- (void)tableView:(UITableView *)tableView
  ...
  if ([tableView isEqual:self.metroSearchController.searchResultsTableView]) {
    ...
    if (!self.isStation) {
      ...
      [self.tvMetroListView reloadSections:[NSIndexSet indexSetWithIndex:0]
...
- (IBAction)didSelectCityBackBtn:(id)sender {
  ...
  if (self.isStation) {
    ...
    /// instantiateViewControllerWithIdentifier:@"cityNavigator"];
    //  [self presentViewController:myView animated:YES completion:nil];
  ...
  for (MetroLineItem *item in self.metroArray) {
    ...
    for (MetroStation *station in item.stationArray) {
      if ([resultPredicate evaluateWithObject:station.stationName]) {
        ...
        info.mixedString = [NSString
            stringWithFormat:@"%@ - %@", item.lineName, station.stationName];
...
- (IBAction)unwindSegueFromCitySelect:(UIStoryboardSegue *)segue {
  CityViewController *controller =
      [[segue.sourceViewController viewControllers] objectAtIndex:0];
...
- (void)onGetPoiResult:(BMKPoiSearch *)searcher
  ...
  if (errorCode == BMK_SEARCH_NO_ERROR) {
    ...
    for (BMKPoiInfo *info in infos) {
      ...
      if (flag) {
        BMKMapPoint point1 =
            BMKMapPointForCoordinate(self.curLocation.location.coordinate);
...
- (BMKAnnotationView *)_mapView:(BMKMapView *)_mapView
